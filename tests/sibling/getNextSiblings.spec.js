...
describe('getNextSiblings() 方法', () => {
  ...
  it('getNextSiblings() 不传递任何参数，返回：[]', () => {
    expect(getNextSiblings()).toEqual([])
  })

  it('getNextSiblings($list) 没有下一个邻居节点，返回：[]', () => {
    expect(getNextSiblings($list)).toEqual([])
  })

  it('getNextSiblings($home) 有下一个邻居节点，返回：邻居节点数组', () => {
    const $home = byId('#item-home')
    const $siblings = getNextSiblings($home)
