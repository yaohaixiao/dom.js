...
describe('getOffset() 方法', () => {
  ...
  it('getOffset() 不传递参数，返回：null', () => {
    expect(getOffset()).toBe(null)
  })

  it('getOffset($list) 返回：40', () => {
    const position = {
      top: 0,
      left: 0
    }
    ...
    position.left = getOffset($list).left
    position.top = getOffset($list).top

    expect(position.left).toEqual(40)
    expect(position.top).toEqual(40)
  })

  it('getOffset($home)，父节点的 offsetLeft=40，offsetTop=40，自身 offsetLeft=20，offsetTop=10，返回：{left: 60, top: 50}', () => {
    const position = {
      top: 0,
      left: 0
    }
    ...
    position.left = getOffset($home).left
    position.top = getOffset($home).top
