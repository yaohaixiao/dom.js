...
describe('hide() 方法', () => {
  ...
  it('hide() 不传递参数，返回：false', () => {
    expect(hide()).toBe(false)
  })

  it(`hide($list) 隐藏 $list，再使用 getStyle($list, 'display') 返回：'none'`, () => {
    hide($list)
    expect(getStyle($list, 'display')).toEqual('none')
  })

  it(`toggle($list) 显示 $list，然后 hide($list) 隐藏 $list，再使用 getStyle($list, 'display') 返回：'none'`, () => {
    toggle($list)
